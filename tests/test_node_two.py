...
from backend.game_state import DayOutcome, initial_game_state
...
class NodeTwoFlowTests(unittest.TestCase):
    def test_golden_path_reveals_finds_returns_and_escapes(self):
        asked = resolve_node_two_turn(
            node_two_state(),
            DmDecision(NodeTwoIntent.ASK_PROBLEM, 1),
        )
        ...
        found = resolve_node_two_turn(
            asked.state,
            DmDecision(NodeTwoIntent.SEARCH_NECKLACE, 2),
        )
        self.assertEqual(found.outcome, DayOutcome.ACTIVE)
        self.assertEqual(state_view(found.state)["necklace"]["status"], "found")
        self.assertIn("crow_necklace", found.state.day.fox.inventory)

        returned = resolve_node_two_turn(
            found.state,
            DmDecision(NodeTwoIntent.RETURN_NECKLACE, 1),
        )
        view = state_view(returned.state)
    ...
    def test_search_without_learning_the_problem_does_not_find_necklace(self):
        result = resolve_node_two_turn(
            node_two_state(),
            DmDecision(NodeTwoIntent.SEARCH_NECKLACE, 2),
        )

        self.assertEqual(state_view(result.state)["necklace"]["status"], "lost")
    ...
    def test_other_action_exposes_only_its_confirmed_public_event(self):
        result = resolve_node_two_turn(
            node_two_state(),
            DmDecision(NodeTwoIntent.OTHER, 1),
        )

        self.assertEqual(result.events, (NodeTwoEvent.TIME_PASSES_HUNGRIER,))
        self.assertEqual(
        ...
        self.assertNotIn("necklace", str(narrative_view(result.state)).casefold())
    ...
    def test_failed_day_resets_world_but_preserves_fox_memory(self):
        ...
        reset = resolve_node_two_turn(
            asked.state,
            DmDecision(NodeTwoIntent.OTHER, 5),
        )
        ...
        self.assertEqual(state_view(reset.state)["necklace"]["status"], "lost")

    def test_dm_payload_is_strict_and_time_bounded(self):
        state = node_two_state()
        decision = parse_dm_decision(
            {"intent": "ask_problem", "timeCost": 1},
            state,
        )
        self.assertEqual(decision.intent, NodeTwoIntent.ASK_PROBLEM)

        with self.assertRaisesRegex(NodeTwoError, "only intent and timeCost"):
            parse_dm_decision(
                {"intent": "ask_problem", "timeCost": 1, "friendship": True},
                state,
            )
        with self.assertRaisesRegex(NodeTwoError, "time remaining"):
            parse_dm_decision(
    ...
    def test_session_commit_detects_stale_turn(self):
        store = NodeTwoSessionStore()
        session_id, original = store.create(loop_count=3)
        updated = resolve_node_two_turn(
            original,
            DmDecision(NodeTwoIntent.ASK_PROBLEM, 1),
        ).state

        store.commit(session_id, original, updated)
        with self.assertRaisesRegex(NodeTwoError, "story changed"):
            store.commit(session_id, original, updated)
